function dat2mat(inDir,outDir)
% CHANGELOG...
% BUILD 9:
% - Use 30 uV threshold for rejection instead of 10% proportion of sweeps
% (= Build 4 of BC_ARTREJ_PLUSMINUS).
% BUILD 10:
% - Allow "inDir" and "outDir" input arguments.
% - Allow slope of "00".
% - Use Build 5 of BC_ARTREJ_PLUSMINUS to be compatible with flat-tone
% naming conventions.

exportInfo.functionSpec = mfilename;
exportInfo.versionSpec = '_10_';

% import functions
addpath(['Z:\Spectral_Dynamics_Grant\FFR\Tone_Glide_Study\Matlab_Code\'...
    'artrej_by_thresh\_1_']);
addpath(['Z:\Spectral_Dynamics_Grant\FFR\Tone_Glide_Study\Matlab_Code\'...
    'bc_artRej_plusMinus\development\_5_']);

%% SECTION 0: OPTIONS & SETTINGS
%% 0.1) Default Directories - CHANGE AS NEEDED
if abs(nargin)<1 || isempty(inDir)
    inDir = ['C:\Users\vhapormadseb\Desktop\Molis FFR dat files\'...
        'tone glide study\CztoA2'];
end
if abs(nargin)<2 || isempty(outDir)
    outDir = [inDir '\mats'];
end

%% 0.2) Artifact Rejection
rejTH = 30; % target rejection threshold in uV
...
for i=1:numel(fList)
    ...
    exportInfo = sortfields(exportInfo);
    params = sortfields(params);
...
bc_artRej_plusMinus(outDir, rejTH, bufferMs);
...
% example format: HI007_R_10_down_120_13_A1ref_eeg.dat
...
if isempty(toks.targ)
    out.montage.channelToAnalyze = 'Cz';
else
