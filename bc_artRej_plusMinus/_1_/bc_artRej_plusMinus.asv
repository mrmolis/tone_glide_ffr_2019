function bc_artRej_plusMinus(outDir, rejRAT, bufferMs)
% Build 1.
%
...
for i=1:numel(fList)
    ...
    save(fPath,'-struct','cond');
    save(rarePath,'-struct','rare');
    ...
    outpath = [fPath(1:end-9) 'minus' fPath(end-6:end)];
    save(outpath,'-struct','minus');
    outpath = [fPath(1:end-9) 'plus' fPath(end-6:end)];
